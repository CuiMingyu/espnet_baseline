...
class Speech2Text:
    ...
    def __call__(
        ...
        # b. Forward Encoder
        logging.info("Start fwd")
        encoder_out, encoder_out_lens, memory, masks = self.asr_model.encode(batch)
        logging.info("End fwd")

        # c. Passed the encoder result and the beam search
        history_state = batch['history_state']
        batch_nbest_hyps, history_state = self.beam_search(encoder_out, encoder_out_lens, history_state)
        ...
        return results, memory, history_state, masks
def update_memory(
        mems: torch.Tensor,
...
def inference(
    ...
    **kwargs,
    ...
    for key, batch in loader:
        idlist.append(key[0])
    first = idlist[0].split('_')[0]
    for i in range(len(idlist)):
        if idlist[i].split('_')[0] == first:
            id2order[idlist[i]] = 0
            first = idlist[i].split('_')[0]
        else:
            id2order[idlist[i - 1]] = 1
            id2order[idlist[i]] = 0
            first = idlist[i].split('_')[0]
    ...
    mems = tuple()
    mem_lens = 0
    history_context_length = 0
    complemented_version = 0
    concat_decoder = False
    masks = tuple()
    pos = tuple()
    history_state = tuple()
    masks = tuple()
        ...
        for keys, batch in loader:
            ...
            try:
                ...
                if kwargs["report_rtf"]:
                    t = time.perf_counter()
                results, mem, history_state, prev_masks = speech2text(batch)
                if len(mem) != 0:
                    mems, current_length, index, masks = update_memory(mems, mem.detach(), history_context_length, end_point, current_length, index, masks, prev_masks) 
                if kwargs["report_rtf"]:
...
def get_parser():
    ...
    group.add_argument("--fix_mem_length", type=dict, default={}, help="transducer conf for beamsearchtransducer")
