...
class AbsTask(ABC):
    ...
    def get_parser(cls) -> config_argparse.ArgumentParser:
        ...
        group.add_argument(
            ...
            type=int_or_none,
            default=None,
            help="Show the valid logs every the number iterations in each epochs at the "
            "training phase. If None is given, it set to 1000 by default.",
        )
        group.add_argument(
            "--use_tensorboard",
            type=str2bool,
        ...
        group.add_argument(
            ...
            type=int,
            default=0,
            help="The maximum history context size for learning history information. "
            "If 0, default is without using history infomation",
        )
        group.add_argument(
            "--complemented_version",
            type=int,
            default=0,
            help="The context is complemented by two version. " 
            "0 represents standard version which only concats context on key and value pair. "
            "1 represents naive version which concats context on query, key and value. ",
        )
        group.add_argument(
            "--concat_decoder",
            type=str2bool,
            default=False,
            help="Determine whether to concat history state of decoder "
            "False is not concating",
        )
        ...
        parser.add_argument(
            ...
            default=None, 
            type=int, 
            help="Attentive pooling for learnging fixed memory length."
        )
        parser.add_argument(
            "--pooling_mode", 
            default="mhap", 
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if not distributed_option.distributed or distributed_option.dist_rank == 0:
            ...
            setup_logging_config(verbose=args.log_level, prefix=f"[{os.uname()[1].split('.')[0]}{_rank}]")
        else:
            # Suppress logging if RANK != 0
            from espnet2.utils import setup_logging_config
            setup_logging_config(verbose="ERROR", prefix=f"[{os.uname()[1].split('.')[0]}"
        ...
        if args.dist_world_size is not None:
            real_ngpu = args.dist_world_size
        else:
            real_ngpu = 1
        args.accum_grad = args.batch_bins // (args.batch_bins_per_gpu * real_ngpu)
        logging.warning(f"Recomputation accum_grad*batch_bins: OLD: {args.batch_bins} NEW: {args.accum_grad}*{args.batch_bins_per_gpu}")
        args.batch_bins = args.batch_bins_per_gpu * real_ngpu
        args.batch_size = args.batch_size * real_ngpu
        ...
        if args.dry_run:
            torch.save(model.state_dict(), output_dir / f"dry_run.pth", _use_new_zipfile_serialization=False)
        ...
        if args.collect_stats:
            ...
            collect_stats(
                ...
                valid_log_interval=args.valid_log_interval,
                write_collected_feats=args.write_collected_feats,
                dry_run="COLLECT" in args.dry_run,
            )
        else:
            ...
            for i, scheduler in enumerate(schedulers, 1):
                if isinstance(scheduler, WarmupLR):
                    ...
                    # name = getattr(args, f"scheduler{suf}")
                    conf = getattr(args, f"scheduler{suf}_conf")
                    if "warmup_ratio" in conf:
                        warmup_epoch = conf["warmup_ratio"] * args.max_epoch
                    else:
                        warmup_epoch = scheduler.warmup_steps * args.accum_grad / len(train_iter_factory.sampler)
                    if "warmup_epoch" in conf:
                        warmup_epoch = conf["warmup_epoch"]
                    # scheduler.warmup_steps = int(warmup_epoch * (len(train_iter_factory.sampler) / args.accum_grad))
                    scheduler.warmup_steps = int(warmup_epoch * len(train_iter_factory.sampler) / args.accum_grad)

                    logging.warning(
                        f"Overwrite warmup_steps by the priority: warmup_epoch > warmup_ratio > warmup_steps\n"
                        f"\tWarmup Steps:{scheduler.warmup_steps} N-batch/accum:{len(train_iter_factory.sampler)}/{args.accum_grad} "
                        f"Estimate epoch to end:{warmup_epoch:.1f}/{args.max_epoch}({warmup_epoch / args.max_epoch:.2f})"
                    )
    ...
    def build_sequence_iter_factory(
        ...
        # record order of each audio id
        
        id2order = {}
        # for gigaspeech
        if mode == 'train' or mode == 'valid': 
            for i in range(iter_options.batch_size):
                first = batches[0][i].split('_')[0]
                for m in range(len(batches)):
                    if batches[m][i].split('_')[0] == first:
                        id2order[batches[m][i]] = 0
                        first = batches[m][i].split('_')[0]
                    else:
                        id2order[batches[m - 1][i]] = 1
                        id2order[batches[m][i]] = 0
                        first = batches[m][i].split('_')[0]
    ...
    def build_model_from_file(
        ...
        if model_file is not None:
            ...
            model.load_state_dict(torch.load(model_file, map_location=device), False)
        else:
            raise ValueError(f"Not exist such model:{model_file}")
        return model, args
