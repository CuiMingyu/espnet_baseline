...
from espnet2.asr.preencoder.downsampling import DownSampling
...
from espnet2.asr.joint_network.base_joint_network import BaseJointNetwork

joint_network_choices = ClassChoices(
    "joint_network",
    classes=dict(base=BaseJointNetwork),
    type_check=AbsJointNetwork,
    default="base",
)
...
class ASRTask(AbsTask):
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetASRModel:
        ...
        preencoder = preencoder_class(input_size=input_size, dropout_rate=args.dropout_rate, **args.preencoder_conf)
        output_size = preencoder.output_size()

        # Encoder
        encoder_class = encoder_choices.get_class(args.encoder)
        encoder = encoder_class(
            output_size=output_size,
            dropout_rate=args.dropout_rate, **args.encoder_conf
        )
        ...
        if args.decoder == "rnn":
            ...
            # Joint Network
            joint_network_class = joint_network_choices.get_class(args.joint_network)
        ...
        #ctc = None
        if args.fix_mem_length is not None:
            pooling = AttentiveStatisticsPooling(args.pooling_mode, args.fix_mem_length, output_size)
            # logging.warning("Using attn pooling memory")
        else:
            pooling = None
        # pooling = None
        # 8. Build model
