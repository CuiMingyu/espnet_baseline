...
class SubReporter:
    ...
    def register(
        ...
        for key2, v in stats.items():
            ...
            if key2 in self._seen_keys_in_the_step and not redo:
            ...
            if key2 not in self.stats:
                ...
                if key2 in self._seen_keys_in_the_step and redo:
                    self.stats[key2][-1] = r
                else:
                    self.stats[key2].append(r)
            self._seen_keys_in_the_step.add(key2)

    def log_message(self, step: int, start: int = None, end: int = None) -> str:
        if self._finished:
            raise RuntimeError("Already finished")
        if start is None:
            start = 0
        ...
        message = f"{self.key}: {self.epoch}-th epoch: {step}-th step: "
    ...
    @contextmanager
    def measure_time(self, name: str, verbose: bool = True):
    ...
    def measure_iter_time(self, iterable, name: str):
        ...
        while True:
            try:
                ...
                t = time.perf_counter() - start
                self.register({name: t}, redo=True)
                yield retval
