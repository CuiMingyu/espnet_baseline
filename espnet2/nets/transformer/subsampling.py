...
class CausalConv2dSubsampling(torch.nn.Module):
    """Causal Convolutional 2D subsampling (to 1/4 length).
    ...
    def __init__(self, idim, odim, dropout_rate, pos_enc=None):
        ...
        self.conv1 = torch.nn.Conv2d(1, odim, 3, 2, 2)
        self.relu1 = torch.nn.ReLU()
        self.conv2 = torch.nn.Conv2d(odim, odim, 3, 2, 2)
        self.relu2 = torch.nn.ReLU()

        self.out = torch.nn.Sequential(
            # out_size = ceil[(n + 2p - k) / s] + 1
            # n: idim
            # p: padding size
            # k: kernel size
            # s: stride size
            torch.nn.Linear(odim * (((idim + 3) // 2 +3) // 2), odim),
    ...
    def forward(self, x, x_mask):
        ...
        x = x[:, :, :-1, :]
        # x shoud be reduced by 4, need to verify
        x_mask = x_mask[:, :, ::2][:, :, ::2]
...
class Conv2dSubsampling(torch.nn.Module):
    ...
    def forward(self, x, x_mask, mems):
        """Subsample x.
        ...
        if len(mems) != 0:
            x_0 = x[0]
            x_0 = torch.concat((mems, x_0), 1)
            x_new_pos = self.pos(x_0)
            x = (x[0], x_new_pos[1])

        # if mems is None:
        #     x = self.out(x.transpose(1, 2).contiguous().view(b, t, c * f))
        # else:
        #     # get new x of current feats
        #     x = self.out(x.transpose(1, 2).contiguous().view(b, t, c * f))
        #     x_0 = x[0]
        #     if len(mems) == 1:
        #         for i in mems:
        #             x_0 = torch.concat((i, x_0), 1)
        #     else:
        #         # reverse mems
        #         mems = mems[::-1]
        #         for i in mems:
