...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def forward(
        ...
        encoder_out, encoder_out_lens = self.encode(batch)
        ...
        return loss, stats, weight, batch['feats'], batch['feats_lengths']
    ...
    def encode(
        ...
        if self.preencoder is not None:
            feats, feats_lengths, masks = self.preencoder(feats, feats_lengths)

        # 4. Forward encoder
        # feats: (Batch, Length, Dim)
        # -> encoder_out: (Batch, Length2, Dim2)
        encoder_out, encoder_out_lens, _ = self.encoder(feats, feats_lengths, masks)
