...
from espnet2.nets.transducer.utils import create_lm_batch_states
from espnet2.nets.transducer.utils import init_lm_state
from espnet2.nets.transducer.utils import is_prefix
from espnet2.nets.transducer.utils import recombine_hyps
from espnet2.nets.transducer.utils import select_k_expansions
from espnet2.nets.transducer.utils import select_lm_state
from espnet2.nets.transducer.utils import subtract
from espnet2.asr.beam_search.scorer_interface import ExtendedHypothesis
...
class BeamSearchTransducer:
    ...
    def to(self, device, dtype):
        self.joint_network.to(device=device, dtype=dtype)
    ...
    def forward(
        self, encoder_out: torch.Tensor, encoder_out_lens: torch.Tensor, history_state=None,
        ...
        """
        assert encoder_out.shape[0] == 1, f"batch_size == 1, but got {encoder_out.shape}"

        nbest_hyps, history_state = self.search_algorithm(encoder_out[0], history_state)

        for hyp in nbest_hyps:
            sos = torch.tensor([0]).long().to(encoder_out.device)
            yseq = torch.tensor(hyp.yseq).long().to(encoder_out.device)
            hyp.yseq = torch.cat((sos, yseq, sos), dim=-1)

        return [nbest_hyps], history_state
    ...
    def greedy_search(self, enc_out: torch.Tensor) -> List[Hypothesis]:
        ...
        """
        dec_state = self.decoder.init_state(1, device=enc_out.device)
    ...
    def default_beam_search(self, enc_out: torch.Tensor, history_state=None) -> List[Hypothesis]:
        ...
        for enc_out_t in enc_out:
            ...
            while True:
                ...
                if max([len(x.yseq) for x in hyps]) >= enc_out.shape[0] + 3:
                    break
        ...
        return self.sort_nbest(kept_hyps), state
    ...
    def time_sync_decoding(self, enc_out: torch.Tensor) -> List[Hypothesis]:
        ...
        beam_state = self.decoder.init_state(beam,device=enc_out.device)
