...
def collect_stats(
    ...
    for itr, mode in zip([train_iter, valid_iter], ["train", "valid"]):
        ...
        with DatadirWriter(output_dir / mode) if not dry_run else DatadirWriter(None) as datadir_writer:
        ...
        with (output_dir / mode / "batch_keys").open("w", encoding="utf-8") if not dry_run else DatadirWriter(None) as f:
            f.write(
                "\n".join(filter(lambda x: not x.endswith("_lengths"), batch)) + "\n"
            )
        with (output_dir / mode / "stats_keys").open("w", encoding="utf-8") if not dry_run else DatadirWriter(None) as f:
            f.write("\n".join(sum_dict) + "\n")
