...
public:
    ...
    rnntStatus_t cost_and_grad(ProbT* log_probs,
                              ProbT* grads,
                              ProbT* costs,
    ...
    rnntStatus_t score_forward(ProbT* log_probs,
                              ProbT* costs,
                              const int* const flat_labels,
                              const int* const label_lengths,
                              const int* const input_lengths);
...
private:
    ...
    public:
        CpuRNNT_metadata(int T, int U, void* workspace, size_t bytes_used, int blank, 
                    const int* const labels, ProbT* log_probs, CpuRNNT_index& idx);
        ProbT* alphas;
        ProbT* betas;
    ...
    private:
        void setup_probs(int T, int U, const int* const labels, int blank, 
                            ProbT* log_probs, CpuRNNT_index& idx);
    };
    ...
    ProbT cost_and_grad_kernel(ProbT* log_probs, ProbT* grad,
                               const int* const labels, int mb,
                               int T, int U, size_t bytes_used);

    ProbT compute_alphas(ProbT* log_probs, int T, int U, ProbT* alphas);
    
    ProbT compute_betas_and_grad(ProbT* grad, ProbT* log_probs,
...
template<typename ProbT>
ProbT
CpuRNNT<ProbT>::cost_and_grad_kernel(ProbT* log_probs, ProbT* grad,
...
template<typename ProbT>
ProbT
CpuRNNT<ProbT>::compute_alphas(ProbT* log_probs, int T, int U, ProbT* alphas) {
...
template<typename ProbT>
ProbT
CpuRNNT<ProbT>::compute_betas_and_grad(ProbT* grad, ProbT* log_probs,
                                int T, int U, ProbT* alphas, ProbT* betas,
                                const int* const labels, ProbT logll) {
...
template<typename ProbT>
rnntStatus_t
CpuRNNT<ProbT>::cost_and_grad(ProbT* log_probs,
                            ProbT* grads,
                            ProbT* costs,
                            const int* const flat_labels,
                            const int* const label_lengths,
...
template<typename ProbT>
rnntStatus_t
CpuRNNT<ProbT>::score_forward(ProbT* log_probs, 
                            ProbT* costs,
                            const int* const flat_labels,
                            const int* const label_lengths,
                            const int* const input_lengths) {
