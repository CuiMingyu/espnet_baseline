...
void log_softmax(float* acts, float* denom, int alphabet_size, int minibatch, int maxT, int maxU, CUstream stream) {
...
rnntStatus_t compute_cost_and_score(float* acts,
                                    float* grads,
                                    float* costs,
        ...
        if (fastemit_lambda > 0.0f) 
        {
            for (int stage=0; stage<=2; stage++)
            {
                compute_fastemit_grad_kernel<128, float><<<minibatch * maxT * maxU, 128, 0, stream>>>(grads, 
                            ...
                            minibatch, maxT, maxU, alphabet_size, blank, fastemit_lambda, stage);
            }

        } 
        else 
        {
            // stage 0: compute non-label and non-blank
            // stage 1: compute (T-1, U-1)
            // stage 2: compute blank and label
            for (int stage=0; stage <=2; stage ++)
            {
                compute_grad_kernel<128, float><<<minibatch * maxT * maxU, 128, 0, stream>>>(grads, 
                    acts, denom, alphas, betas, llForward, input_lengths, label_lengths, labels, 
                    minibatch, maxT, maxU, alphabet_size, blank, stage);
...
rnntStatus_t cost_and_grad_gpu(float* acts,
                       float* grads,
                       float* costs,
...
rnntStatus_t score_forward_gpu(float* acts,
                       float *costs,
                       const int* const pad_labels,
                       const int* const label_lengths,
                       const int* const input_lengths,
