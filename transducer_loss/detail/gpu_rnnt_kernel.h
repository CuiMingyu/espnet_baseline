...
__global__ void compute_alphas_kernel(Tp* acts, const Tp* const denom, Tp* alphas, Tp* llForward, const int* const xlen, const int* const ylen, 
...
template<typename Tp>
__global__ void compute_alphas_kernel_naive(Tp* acts, const Tp* const denom, Tp* alphas, Tp* llForward, const int* const xlen, const int* const ylen, 
    const int* const mlabels, const int minibatch, const int maxT, const int maxU, const int alphabet_size, const int blank_) {
...
template<typename Tp>
__global__ void compute_betas_kernel(Tp* acts, const Tp* const denom, Tp* betas, Tp* llBackward, const int* const xlen, const int* const ylen, 
    const int* const mlabels, const int minibatch, const int maxT, const int maxU, const int alphabet_size, const int blank_) {
    int b = blockIdx.x; // batch
    int u = threadIdx.x; // label id, u
    const int T = xlen[b];
...
template<typename Tp>
__global__ void compute_betas_kernel_naive(Tp* acts, const Tp* const denom, Tp* betas, Tp* llBackward, const int* const xlen, const int* const ylen, 
    const int* const mlabels, const int minibatch, const int maxT, const int maxU, const int alphabet_size, const int blank_) {
    int tid = threadIdx.x; // mb
    const int T = xlen[tid];
    const int U = ylen[tid] + 1;
...
__global__ void compute_grad_kernel(Tp* grads, Tp* acts, const Tp* const denom, const Tp* alphas, const Tp* betas, const Tp* const logll, const int* const xlen, const int* const ylen, 
    const int* const mlabels, const int minibatch, const int maxT, const int maxU, const int alphabet_size, const int blank_, int stage) {
    ...
    if (t < T && u < U)
    {
        while (idx < alphabet_size) {
            ...
            if (stage == 0)
            {
                if ( (idx == blank_ && t == T-1 && u == U-1) || (idx == blank_ && t < T-1) || (u < U-1 && idx == labels[u]) )
                {
                    ;
                }
                else
                {
                    acts[col * alphabet_size + idx] = grad;
                    // grads[col * alphabet_size + idx] = grad;
                }
            }
            // grad to last blank transition
            if (idx == blank_ && t == T-1 && u == U-1 && stage == 1)
            {
                grad -= exp(alphas[col] + logpk - logll[mb]);
                acts[col * alphabet_size + idx] = grad;
                // grads[col * alphabet_size + idx] = grad;
            }
            if (idx == blank_ && t < T-1 && stage == 2)
            {
                grad -= exp(alphas[col] + logpk - logll[mb] + betas[col + maxU]);
                acts[col * alphabet_size + idx] = grad;
                // grads[col * alphabet_size + idx] = grad;
            }
            if (u < U-1 && idx == labels[u] && stage == 2)
            {
...
template<int NT, typename Tp>
__global__ void compute_fastemit_grad_kernel(Tp* grads, Tp* acts, const Tp* const denom,
                                            ...
                                            const int maxU, const int alphabet_size, const int blank_, 
                                            const Tp fastemit_lambda, int stage) 
{
    int tid = threadIdx.x; // alphabet dim
    int idx = tid;
    int col = blockIdx.x; // mb, t, u
    ...
    if (t < T && u < U) 
    {
        while (idx < alphabet_size) 
        {
            ...
            Tp logy_btu1 = rnnt_helper::neg_inf<Tp>(); // log(y(t,u)) + log(beta(t, u+1))
            if (u < U-1) 
            {
            ...
            if (stage == 0)
            {
                ...
                else if (idx == blank_ && t < T-1) 
                {
                    ;
                }
                else if (u < U-1 && idx == labels[u]) 
                {
                    ;
                }
                else
